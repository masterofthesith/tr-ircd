...
/*
 * $Id: dh.c,v 1.4 2003/06/14 13:55:52 tr-ircd Exp $ 
 */
...
int i;
    ...
    printf("No random state found, trying to generate entropy from %s...\n", RANDFILE);
    printf("This may take a moment.\n");
    printf("To speed up this process, do something else on the system for a bit.\n");

    i = RAND_load_file(RANDFILE, 1024);

    if (i <= 0) {
	printf("Could not load random values from %s (file not found?)\n", RANDFILE);
	printf("Use the \02irandom.sh\02 utility to create a random data file\n");
	printf("ircd needs a %d byte random seed.\n", RAND_BYTES);
	printf("You can place a file containing random data called ircd.entropy\n");
	printf("in the var/ directory of your installation. This file must be at least %d bytes\n",
	       RAND_BYTES);
...
time_t now;
    ...
    ret = RAND_load_file(ENTROPYFILE, -1);
    if (ret <= 0) {
	if (!make_entropy())
	    return -1;
    } else
...
int dh_generate_shared(struct dh_session *si, char *public_key)
{
    BIGNUM *tmp;
    int len;
...
struct dh_session *dh_start_session(void)
{
    struct dh_session *si = (struct dh_session *) malloc(sizeof(struct dh_session));
    if (!si)
	abort();
...
void dh_end_session(struct dh_session *si)
{
    if (si->dh) {
	DH_free(si->dh);
	si->dh = NULL;
...
char *dh_get_s_public(char *buf, int maxlen, struct dh_session *si)
{
    char *tmp;
...
int dh_get_s_shared(char *buf, int *maxlen, struct dh_session *si)
{

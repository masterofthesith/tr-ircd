 ...
 *
 * $Id: types.c,v 1.3 2005/09/30 09:28:58 tr-ircd Exp $
 */
...
static int search_sortlist(adns_state ads, struct in_addr ad)
{
    ...
    for (i = 0, slp = ads->sortlist;
	 i < ads->nsortlist && !((ad.s_addr & slp->mask.s_addr) == slp->base.s_addr); i++, slp++);
    return i;
}
...
static adns_status csp_addr(vbuf * vb, const adns_rr_addr * rrp)
{
    ...
    char ip[IP6STRLEN];
		CSP_ADDSTR("INET6 ");
		assert(inetntop
		       (AF_INET6, rrp->addr.inet6.sin6_addr.s6_addr, ip, IP6STRLEN) != NULL);
		CSP_ADDSTR(ip);
		break;
	    }
#endif
			      ...
			      char **domain_r, parsedomain_flags flags)
{
    ...
    st = adns__parse_domain(pai->qu->ads, pai->serv, pai->qu, &pai->qu->vb,
			    flags, pai->dgram, pai->dglen, cbyte_io, max);
    if (st)
	return st;
    if (!pai->qu->vb.used)
	return adns_s_invaliddata;
				 ...
				 int *cbyte_io, int count, int dmstart)
{
	...
	st = adns__findrr_anychk(pai->qu, pai->serv, pai->dgram, pai->dglen,
				 cbyte_io, &type, &class, &ttl, &rdlen,
				 &rdstart, pai->dgram, pai->dglen, dmstart, &ownermatched);
	if (st)
	    return st;
	...
	adns__isort(ha->addrs, naddrs, sizeof(adns_rr_addr), pai->qu->vb.buf, div_addr, pai->ads);
    }
    return adns_s_ok;
}
				...
				int max, adns_rr_hostaddr * rrp)
{
    ...
    *cbyte_io = cbyte;
    ...
    st = adns__mkquery_frdgram(pai->ads, &pai->qu->vb, &id,
			       pai->dgram, pai->dglen, dmstart, adns_r_addr, adns_qf_quoteok_query);
    if (st)
	return st;
...
static int dip_hostaddr(adns_state ads, const adns_rr_hostaddr * ap, const adns_rr_hostaddr * bp)
{
    ...
    return dip_inaddr(ads, ap->addrs[0].addr.inet.sin_addr, bp->addrs[0].addr.inet.sin_addr);
}
...
int i;
	...
	if (queried->len == found->len && !memcmp(&queried->addr, &found->addr, queried->len)) {
	    if (!parent->children.head) {
		adns__query_done(parent);
		return;
	    } else {
...
static adns_status pa_ptr6(const parseinfo * pai, int dmstart, int max, void *datap)
{
    ...
    st = adns__internal_submit(pai->ads, &nqu, adns__findtype(adns_r_addr6),
			       &pai->qu->vb, id, adns_qf_quoteok_query, pai->now, &ctx);
    if (st)
	return st;
...
static adns_status pap_mailbox822(const parseinfo * pai, int *cbyte_io, int max, char **mb_r)
{
    ...
    adns__findlabel_start(&fls, pai->ads,
			  -1, pai->qu, pai->dgram, pai->dglen, max, *cbyte_io, cbyte_io);
    st = adns__findlabel_next(&fls, &lablen, &labstart);
    if (!lablen) {

 ...
 *
 * $Id: check.c,v 1.3 2005/09/30 09:28:58 tr-ircd Exp $
 */
...
#define DLIST_ASSERTON_PARTLESS(node, nodevar, list)                            \
  do {                                                                          \
    for ((nodevar)= (list).head;                                                \
         (nodevar) != (node);                                                   \
...
adns_query parent, child;
    ...
    DLIST_CHECK_PARTLESS(ads->childw, parent, {
			 ...
			 checkc_query(ads, parent); checkc_query_alloc(ads, parent);});
}
...
adns_query search;
	...
	switch (qu->state) {
	    case query_tosend:
		DLIST_ASSERTON_PARTLESS(qu, search, ads->udpw);
		break;
	    case query_tcpw:
		DLIST_ASSERTON_PARTLESS(qu, search, ads->tcpw);
		break;
	    case query_childw:
		DLIST_ASSERTON_PARTLESS(qu, search, ads->childw);
		break;
	    case query_done:
		DLIST_ASSERTON_PARTLESS(qu, search, ads->output);
		break;
	    default:

 ...
 *
 * $Id: reply.c,v 1.3 2005/09/30 09:28:58 tr-ircd Exp $
 */
		     ...
		     int serv, int viatcp, struct timeval now)
{
	...
	if (!qdcount) {
	    ...
	    adns__diag(ads, serv, 0,
		       "server claimed to answer %d questions with one message", qdcount);
	...
	st = adns__findrr(qu, serv, dgram, dglen, &cbyte,
			  &rrtype, &rrclass, &ttl, &rdlength, &rdstart, &ownermatched);
	if (st) {
	...
	if (rrtype == adns_r_cname && (qu->typei->type & adns__rrt_typemask) != adns_r_cname) {
		...
		adns__debug(ads, serv, qu,
			    ...
			    qu->answer->cname, adns__diag_domain(ads, serv, qu,
								 &qu->vb, dgram, dglen, rdstart));
		adns__query_fail(qu, adns_s_prohibitedcname);
		return;
		...
		st = adns__parse_domain(ads, serv, qu, &qu->vb,
					...
					pdf_quoteok, dgram, dglen, &rdstart, rdstart + rdlength);
		if (!qu->vb.used)
		    goto x_truncated;
		if (st) {
		    adns__query_fail(qu, st);
  ...
  x_restartquery:
	...
	newquery = (byte *) MyRealloc(qu->query_dgram, qu->vb.used);
	if (!newquery) {
	    adns__query_fail(qu, adns_s_nomemory);
	    return;
	}

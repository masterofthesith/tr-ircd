 ...
 *
 * $Id: setup.c,v 1.3 2005/09/30 09:28:58 tr-ircd Exp $
 */
...
static void addserver(adns_state ads, struct in_addr addr)
{
	...
	if (ads->servers[i].addr.s_addr == addr.s_addr) {
	    adns__debug(ads, -1, 0, "duplicate nameserver %s ignored", inetntoa((char *) &addr));
	    return;
	}
    }
...
static void ccf_search(adns_state ads, const char *fn, int lno, const char *buf)
{
    ...
    newptrs = (char **) MyMalloc(sizeof(char *) * count);
    if (!newptrs) {
	saveerr(ads, errno);
	return;
    }
    newchars = (char *) MyMalloc(tl);
    if (!newchars) {
	saveerr(ads, errno);
	MyFree(newptrs);
	return;
			      ...
			      getline_ctx gl_ctx)
{
	...
	for (ccip = configcommandinfos;
	     ccip->name && !(strlen(ccip->name) == dirl && !memcmp(ccip->name, p, q - p)); ccip++);
	if (!ccip->name) {
	    adns__diag(ads, -1, 0,
		       "%s:%d: unknown configuration directive `%.*s'", filename, lno, q - p, p);
	    continue;
	}
...
adns_state ads;
    ...
    ads = (adns_state) MyMalloc(sizeof(*ads));
...
void adns_forallqueries_begin(adns_state ads)
{
	...
	ads->tcpw.head ? ads->tcpw.head : ads->childw.head ? ads->childw.head : ads->output.head;
}

 ...
 *
 * $Id: general.c,v 1.3 2005/09/30 09:28:58 tr-ircd Exp $
 */
		 ...
		 int serv, adns_query qu, const char *fmt, va_list al)
{
	...
	fprintf(ads->diagfile, "%sQNAME=%s, QTYPE=%s",
		...
		adns__diag_domain(qu->ads, -1, 0, &vb,
				  ...
				  DNS_HDRSIZE), qu->typei ? qu->typei->rrtname : "<unknown>");
...
int adns__vbuf_append(vbuf * vb, const byte * ddata, int len)
{
	...
	if (newlen < 20)
	    newlen = 20;
	newlen <<= 1;
	nb = (void *) MyRealloc(vb->buf, newlen);
	if (!nb) {
	    newlen = vb->used + len;
	    nb = (void *) MyRealloc(vb->buf, newlen);
	}
			      ...
			      vbuf * vb, const byte * dgram, int dglen, int cbyte)
{
	...
	if (!(adns__vbuf_appendstr(vb, "<bad format... ") &&
	      ...
	      adns__vbuf_appendstr(vb, ">") && adns__vbuf_append(vb, (const byte *) "", 1))) {
			 ...
			 int *len_r, const void *datap, char **data_r)
{
    ...
    *data_r = (char *) MyRealloc(vb.buf, vb.used);
    if (!*data_r)
	*data_r = (char *) vb.buf;
    return adns_s_ok;
...
static const struct stinfo {
    ...
    adns_s_ok, "ok"},
	STINFO(localfail),
	STINFO(remotefail), STINFO(tempfail), STINFO(misconfig), STINFO(misquery), STINFO(permfail)
};
...
void adns__isort(void *array, int nobjs, int sz, void *tempbuf,
		 int (*needswap) (void *context, const void *a, const void *b), void *context)
{
    byte *ddata = array;
    int i, place;
    ...
    for (i = 0; i < nobjs; i++) {
	for (place = i;
	     place > 0 && needswap(context, ddata + (place - 1) * sz, ddata + i * sz); place--);
	if (place != i) {

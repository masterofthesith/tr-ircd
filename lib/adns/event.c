 ...
 *
 * $Id: event.c,v 1.3 2005/09/30 09:28:58 tr-ircd Exp $
 */
...
int adns__pollfds(adns_state ads, struct adns_pollfd pollfds_buf[MAX_POLLFDS])
{
	...
	case server_ok:
		...
		ads->tcpsend.
		used ? ADNS_POLLIN | ADNS_POLLOUT | ADNS_POLLPRI : ADNS_POLLIN | ADNS_POLLPRI;
	    break;
	default:
	    abort();
    }
...
int adns_processreadable(adns_state ads, int fd, const struct timeval *now)
{
	...
	for (;;) {
		...
		adns__diag(ads, -1, 0,
			   ...
			   " (expected %lu)", udpaddrlen, (unsigned long) sizeof(udpaddr));
		continue;
	    }
...
static void fd_event(adns_state ads, int fd,
		     ...
		     int (*func) (adns_state, int fd,
				  const struct timeval * now), struct timeval now, int *r_r)
{
    int r;
...
void adns__fdevents(adns_state ads,
		    ...
		    const fd_set * writefds, const fd_set * exceptfds, struct timeval now, int *r_r)
{
    int i, fd, revents;
...
void adns_beforeselect(adns_state ads, int *maxfd_io, fd_set * readfds_io,
		       ...
		       struct timeval **tv_mod, struct timeval *tv_tobuf, const struct timeval *now)
{
...
int adns__internal_check(adns_state ads,
			 adns_query * query_io, adns_answer ** answer, void **context_r)
{
    adns_query qu;
